...
class ChongQing2018l(object):
    def __init__(self):
        """初始化数据"""
        # 准备url模板
        self.url = ConfigUtil().get("OCOP", "CHONGQING_2018_L")
        # 省份名称
        self.sydsfmc = "重庆"
        # 录取年份
        self.lqnf = 2018
    ...
    def get_datas_from_page(self, page):
        ...
        # 获取标签列表
        # xpath返回的是一个列表
        trs = element.xpath("//*/table[1]/tbody/tr")
        # 遍历列表,提取需要的数据
        data_list = []
        i = 0
        for tr in trs:
            ...
            item["klmc"] = self.klmc
            # 分数段
            item["fssx"] = int("".join(tr.xpath("./td[1]//text()")).strip().replace("及以上", ""))
            item["fsxx"] = item["fssx"]
            # 本分数段人数
            item["bfsdrs"] = int("0" if "".join(tr.xpath("./td[2]//text()")).strip() == "" else "".join(
