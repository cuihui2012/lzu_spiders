...
class ChongQing2016l(object):
    def __init__(self):
        """初始化数据"""
        # 准备url模板
        self.url = ConfigUtil().get("OCOP", "CHONGQING_2016_L")
        # 省份名称
        self.sydsfmc = "重庆"
        # 录取年份
        self.lqnf = 2016
    ...
    def get_datas_from_page(self, page):
        ...
        for tr in trs:
            ...
            item["klmc"] = self.klmc
            # 分数段
            item["fssx"] = int("".join(tr.xpath("./td[2]/p//text()")).strip().replace("及以上", ""))
            item["fsxx"] = item["fssx"]
            # 本分数段人数
            item["bfsdrs"] = int("0" if "".join(tr.xpath("./td[3]/p//text()")).strip() == "" else "".join(
                tr.xpath("./td[3]/p//text()")).strip())
            # 本分数段累计人数
            item["bfsdljrs"] = int("".join(tr.xpath("./td[4]/p//text()")).strip())
