...
class GanSu2020w(object):
    def __init__(self):
        """初始化数据"""
        # 准备url模板
        self.url = ConfigUtil().get("OCOP", "GANSU_2020_W")
        # 省份名称
        self.sydsfmc = "甘肃"
        # 录取年份
        self.lqnf = 2020
    ...
    def get_datas_from_page(self, page):
        ...
        for tr in trs:
            ...
            item["klmc"] = self.klmc
            # 分数段
            item["fssx"] = int("".join(tr.xpath("./td[2]//text()")).strip())
            item["fsxx"] = int("".join(tr.xpath("./td[2]//text()")).strip())
            # 本分数段人数(本分数累计人数-上一分数累计人数)
            if i == 3:
                item["bfsdrs"] = int("".join(tr.xpath("./td[3]//text()")).strip())
            else:
                item["bfsdrs"] = int("".join(tr.xpath("./td[3]//text()")).strip()) - int(
                    "".join(trs[i - 2].xpath("./td[3]//text()")).strip())
            # 本分数段累计人数
            item["bfsdljrs"] = int("".join(tr.xpath("./td[3]//text()")).strip())
