...
class BeiJing2019(object):
    def __init__(self):
        """初始化数据"""
        # 准备url模板
        self.url = ConfigUtil().get("OCOP", "BEIJING_2019")
        # 省份名称
        self.sydsfmc = "北京"
        # 录取年份
        self.lqnf = 2019
    ...
    def get_datas_from_page(self, page):
        ...
        for tr in trs:
            ...
            item["klmc"] = "文史类"
            # 分数段
            range_score = "".join(tr.xpath("./td[1]//text()")).replace("以上", "").split("→")
            ...
            # 本分数段累计人数
            item["bfsdljrs"] = int(tr.xpath("./td[3]//text()")[0].strip())
        ...
        for tr in trs:
            ...
            if i in range(5, 25):
                ...
                # 分数段
                range_score = "".join(tr.xpath("./td[4]//text()")).replace("分以上", "").split("→")
                if len(range_score) == 2:
                    # 分数上限
                    item["fssx"] = int(range_score[1].strip())
                    # 分数下限
                ...
                # 本分数段累计人数
                item["bfsdljrs"] = int(tr.xpath("./td[6]//text()")[0].strip())
            logger.info("记录：%s---->%s" % (i, item))
            data_list.append(item)
        return data_list
