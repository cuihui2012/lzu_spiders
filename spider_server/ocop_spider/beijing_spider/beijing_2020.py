...
class BeiJing2020(object):
    def __init__(self):
        """初始化数据"""
        # 准备url模板
        self.url = ConfigUtil().get("OCOP", "BEIJING_2020")
        # 省份名称
        self.sydsfmc = "北京"
        # 录取年份
        self.lqnf = 2020
    ...
    def get_datas_from_page(self, page):
        ...
        for tr in trs:
            ...
            item["klmc"] = self.klmc
            # 分数段
            range_score = tr.xpath("./td[1]/p//text()")
            if len(range_score) == 3:
                # 分数上限
                item["fssx"] = int(tr.xpath("./td[1]/p//text()")[2].strip())
                # 分数下限
                item["fsxx"] = int(tr.xpath("./td[1]/p//text()")[0].strip())
            else:
                item["fssx"] = int(tr.xpath("./td[1]/p//text()")[0].strip())
                item["fsxx"] = int(tr.xpath("./td[1]/p//text()")[0].strip())
            # 本分数段人数
            item["bfsdrs"] = int(tr.xpath("./td[2]/p//text()")[0].strip())
            # 本分数段累计人数
            item["bfsdljrs"] = int(tr.xpath("./td[3]/p//text()")[0].strip())
